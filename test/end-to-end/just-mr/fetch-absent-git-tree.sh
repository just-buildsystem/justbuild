...
# Now, on a fresh local build root, use the description without any
# absent hint. In this way, we verify that even for concrete repositories
# fetching over the just-serve instance works (as we removed all other ways
# to get the commit).
cat > repos.json <<EOF
{ "repositories":
  { "":
